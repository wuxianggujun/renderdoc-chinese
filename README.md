...
该项目为个人翻译，因为需要使用该工具，所以翻译成中文方便使用。可能有一些翻译不全或者不知道应该翻译成什么，请谅解。该项目只会有Window版本。

RenderDoc 是一个基于帧捕捉的图形调试器,当前支持在 Windows、Linux、Android 和 Nintendo SwitchTM 上进行 Vulkan、D3D11、D3D12、OpenGL 和 OpenGL ES 开发。它是完全开源的,采用 MIT 许可协议。

RenderDoc 旨在仅用于调试您自己的程序。任何关于捕获您未创建的程序的讨论,在任何官方公共 RenderDoc 场合(包括问题跟踪器、Discord 或电子邮件)都不被允许。例如,这包括捕获您未创建的商业游戏,或捕获 Google Maps 或 Google Earth。注意:捕获您创建的项目(使用第三方引擎如 Unreal 或 Unity,或开放源代码和免费的项目)是完全可以接受并支持的。

如果您有任何问题、建议或问题,您可以在 [create an issue](https://github.com/baldurk/renderdoc/issues/new/choose) github, [email me directly](mailto:baldurk@baldurk.org) or come into [IRC](https://webchat.oftc.net/?channels=renderdoc) or [Discord](https://discord.gg/ahq6yRB) to discuss it.
